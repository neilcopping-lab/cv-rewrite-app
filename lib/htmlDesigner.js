...
const DESIGNER_TEMPLATES = [
  { id: "d-executive-navy", name: "Executive Navy", kind: "designer", category: "Designer",
    description: "Right-hand navy panel with key achievements and skills. Bold and senior." },
  { id: "d-teal-sidebar", name: "Teal Sidebar", kind: "designer", category: "Designer",
    description: "Left teal sidebar with a monogram, contact, achievements and skills." },
  { id: "d-header-band", name: "Header Band", kind: "designer", category: "Designer",
    description: "A full-width colour header, achievement cards and skill pills." },
  { id: "d-minimal-elegant", name: "Minimal Elegant", kind: "designer", category: "Designer",
    description: "Centered serif, formal and understated — an Ivy-League feel." },
  { id: "d-timeline", name: "Timeline", kind: "designer", category: "Designer",
...
// Infographic "how I match" block: a count badge + check-marked requirement→proof
// rows, coloured in the template's accent. Uses the candidate's real skills-match.
function matchHtml(cv, accent) {
  const rows = (cv.skillsMatch || []).filter((m) => m && (m.requirement || m.proof));
  if (!rows.length) return "";
  const badge = `<div style="width:46px;height:46px;border-radius:50%;background:${accent};color:#fff;font-size:17pt;font-weight:700;text-align:center;line-height:46px;flex:0 0 auto">${rows.length}</div>`;
  const stat = `<div style="display:flex;align-items:center;gap:12px;margin:2px 0 11px">${badge}<div style="font-size:9pt;color:#555;line-height:1.35">of the advert's key requirements, each backed by real experience from my career</div></div>`;
  const items = rows.map((m) =>
    `<div style="display:flex;align-items:flex-start;margin-bottom:7px"><span style="display:inline-block;width:16px;height:16px;line-height:15px;text-align:center;border-radius:50%;background:${accent};color:#fff;font-size:8.5pt;margin-right:10px;flex:0 0 16px">&#10003;</span><div style="flex:1"><span style="font-weight:700;font-size:9.7pt;color:#1a1a1a">${esc(m.requirement)}</span>${m.proof ? ` <span style="color:#666;font-size:9pt">&mdash; ${esc(m.proof)}</span>` : ""}</div></div>`
  ).join("");
  return `<h2>How I Match This Role</h2>${stat}<div>${items}</div>`;
}
...
// ---- the five templates ----------------------------------------------------
function tNavy(cv) {
  const css = `body{background:linear-gradient(to right,#ffffff 0,#ffffff 62%,#1F3346 62%,#1F3346 100%)}
  ...
  const body = `<table><tr><td class="main"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><div class="contacts">${contactsIconRow(cv)}</div><h2>Summary</h2><div>${esc(cv.personalStatement)}</div>${matchHtml(cv, "#1F3346")}<h2>Experience</h2>${expHtml(cv)}<h2>Education</h2>${eduHtml(cv)}</td><td class="side"><h2>Key Achievements</h2>${achSide(cv)}<h2>Skills</h2>${skillLines(cv)}</td></tr></table>`;
  return pageWrap(css, body);
}
function tTeal(cv) {
  const mono = (cv.header?.name || "").split(/\s+/).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  const c = contactBits(cv);
  const css = `body{background:linear-gradient(to right,#14707E 0,#14707E 35%,#ffffff 35%,#ffffff 100%)}
  ...
  const body = `<table><tr><td class="side"><div class="mono">${esc(mono)}</div><h2>Contact</h2>${cont}<h2>Key Achievements</h2>${achSide(cv)}<h2>Skills</h2>${skillLines(cv)}</td><td class="main"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><h2>Summary</h2><div>${esc(cv.personalStatement)}</div>${matchHtml(cv, "#14707E")}<h2>Experience</h2>${expHtml(cv)}<h2>Education</h2>${eduHtml(cv)}</td></tr></table>`;
  return pageWrap(css, body);
}
function tBand(cv) {
  ...
  const body = `<div class="hdr"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><div class="contacts">${contactsIconRow(cv)}</div></div><div class="wrap"><h2>Summary</h2><div>${esc(cv.personalStatement)}</div>${ach.length ? "<h2>Key Achievements</h2>" + achBlock : ""}${matchHtml(cv, "#2C7A8C")}<h2>Skills</h2>${skillPills(cv)}<h2>Experience</h2>${expHtml(cv, { datesRight: true })}<h2>Education</h2>${eduHtml(cv)}</div>`;
  return pageWrap(css, body);
}
function tMinimal(cv) {
  ...
  const body = `<div class="wrap"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><div class="contacts">${cbits}</div><h2>Profile</h2><div style="text-align:justify">${esc(cv.personalStatement)}</div>${matchHtml(cv, "#8a6a10")}<h2>Experience</h2>${expHtml(cv)}<h2>Core Skills</h2><div class="skl">${skl}</div><h2>Education</h2>${eduHtml(cv)}</div>`;
  return pageWrap(css, body);
}
function tTimeline(cv) {
  ...
  const body = `<div class="wrap"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><div class="contacts">${contactsIconRow(cv)}</div><h2>Summary</h2><div>${esc(cv.personalStatement)}</div>${ach ? "<h2>Key Achievements</h2>" + ach : ""}${matchHtml(cv, "#2C7A8C")}<h2>Experience</h2><div class="tl">${expHtml(cv, { timeline: true })}</div><h2>Skills</h2>${skillPills(cv)}<h2>Education</h2>${eduHtml(cv)}</div>`;
  return pageWrap(css, body);
}
