...
function matchHtml(cv, accent) {
  ...
  const items = rows.map((m) =>
    `<div style="display:flex;align-items:flex-start;margin-bottom:7px"><span style="display:inline-block;width:16px;height:16px;line-height:15px;text-align:center;border-radius:50%;background:${accent};color:#fff;font-size:8.5pt;margin-right:10px;flex:0 0 16px">&#10003;</span><div style="flex:1"><span style="font-weight:700;font-size:9.7pt;color:#1a1a1a">${esc(m.requirement)}</span>${m.proof ? ` <span style="color:#666;font-size:9pt"> - ${esc(m.proof)}</span>` : ""}</div></div>`
  ).join("");
