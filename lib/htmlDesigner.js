...
function matchHtml(cv, accent) {
  ...
  const items = rows.map((m) =>
    `<div style="display:flex;align-items:flex-start;margin-bottom:7px"><span style="display:inline-block;width:16px;height:16px;line-height:15px;text-align:center;border-radius:50%;background:${accent};color:#fff;font-size:8.5pt;margin-right:9px;flex:0 0 auto">&#10003;</span><div><span style="font-weight:700;font-size:9.7pt;color:#1a1a1a">${esc(m.requirement)}</span>${m.proof ? ` <span style="color:#666;font-size:9pt">&mdash; ${esc(m.proof)}</span>` : ""}</div></div>`
  ).join("");
...
// ---- the five templates ----------------------------------------------------
function tNavy(cv) {
  const css = `.sb{position:absolute;top:0;right:0;width:38%;height:100vh;background:#1F3346;z-index:-1}
  ...
  const body = `<div class="sb"></div><table><tr><td class="main"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><div class="contacts">${contactsIconRow(cv)}</div><h2>Summary</h2><div>${esc(cv.personalStatement)}</div>${matchHtml(cv, "#1F3346")}<h2>Experience</h2>${expHtml(cv)}<h2>Education</h2>${eduHtml(cv)}</td><td class="side"><h2>Key Achievements</h2>${achSide(cv)}<h2>Skills</h2>${skillLines(cv)}</td></tr></table>`;
  return pageWrap(css, body);
}
function tTeal(cv) {
  const mono = (cv.header?.name || "").split(/\s+/).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  const c = contactBits(cv);
  const css = `.sb{position:absolute;top:0;left:0;width:35%;height:100vh;background:#14707E;z-index:-1}
  ...
  const body = `<div class="sb"></div><table><tr><td class="side"><div class="mono">${esc(mono)}</div><h2>Contact</h2>${cont}<h2>Key Achievements</h2>${achSide(cv)}<h2>Skills</h2>${skillLines(cv)}</td><td class="main"><div class="name">${esc(cv.header?.name)}</div><div class="role">${esc(cv.header?.targetRole)}</div><h2>Summary</h2><div>${esc(cv.personalStatement)}</div>${matchHtml(cv, "#14707E")}<h2>Experience</h2>${expHtml(cv)}<h2>Education</h2>${eduHtml(cv)}</td></tr></table>`;
  return pageWrap(css, body);
}
function tBand(cv) {
