...
function preview(cv, designId) {
  ...
  const body = `
    ...
    ${(cv.skillsMatch || []).length ? H("How I match this role") + (cv.skillsMatch || []).map((m) => `<p style="margin:4px 0"><strong>${esc(m.requirement)}</strong> <span style="color:#726C5E"> - ${esc(m.proof)}</span></p>`).join("") : ""}
