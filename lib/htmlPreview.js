...
function preview(cv, designId) {
  ...
  const links = [cv.header?.linkedin, cv.header?.portfolio, cv.header?.website, cv.header?.github, cv.header?.introVideo].filter(Boolean);
  ...
  const body = `
    ...
    ${(cv.skillsMatch || []).length ? H("How I match this role") + (cv.skillsMatch || []).map((m) => `<p style="margin:4px 0"><strong>${esc(m.requirement)}</strong> <span style="color:#726C5E">&mdash; ${esc(m.proof)}</span></p>`).join("") : ""}
