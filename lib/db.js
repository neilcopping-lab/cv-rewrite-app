...
function addFeedback({ stars, comment, email, role, context }) {
  const db = load();
  const entry = {
    ...
    context: String(context || "").slice(0, 60) || null,
...
function listFeedback() { return load().feedback.slice().reverse(); } // newest first
